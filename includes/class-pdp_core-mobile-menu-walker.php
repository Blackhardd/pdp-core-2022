...
class PDP_Core_Mobile_Menu_Walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ){
		...
		$args->walker->has_children && $chevron = '<button><svg width="11" height="6" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M.97.54a.65.65 0 0 1 .91 0l3.55 3.54L8.97.54a.65.65 0 0 1 .91.92l-4 4a.65.65 0 0 1-.91 0l-4-4a.65.65 0 0 1 0-.92Z" /></svg></button>';

		$description = ( !empty( $item->description ) && 0 == $depth ) ? '<small class="nav_desc">' . esc_attr( $item->description ) . '</small>' : '';

		$title = apply_filters( 'the_title', $item->title, $item->ID );

		$item_output = '<div class="menu-item-link">' . $args->before . "<a $attributes>" . $args->link_before . $title . '</a> ' . $args->link_after . $description . $args->after . $chevron . '</div>';
